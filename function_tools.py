...
class MyVectorDBConnector:
    ...
    def get_embeddings(self, texts, model=OPENAI_EMBEDDING_SMALL):
        '''封装 OpenAI 的 Embedding 模型接口'''
        data = self.client.embeddings.create(input=texts, model=model).data
        return [x.embedding for x in data]

    # 模型对一次能处理的文本条数有限制，新增batch_size参数加以控制。
    def get_embeddings_batch(self,texts, model=OPENAI_EMBEDDING_SMALL, batch_size=10):
        all_embeddings = []
...
# 访问大模型
def get_completion(prompt, model=OPENAI_GPT_MODEL):
    '''封装 openai 接口'''
